...
def random_blur_kernel(probs, N_blur, sigrange_gauss, sigrange_line, wmin_line):
    ...
    sig_gauss = tf.random_uniform([], sigrange_gauss[0], sigrange_gauss[1])
    ...
    sig_line = tf.random_uniform([], sigrange_line[0], sigrange_line[1])
    w_line = tf.random_uniform([], wmin_line, .5 * (N-1) + .1)
...
def get_rnd_brightness_tf(rnd_bri, rnd_hue, batch_size):
    rnd_hue = tf.random_uniform((batch_size,1,1,3), -rnd_hue, rnd_hue)
    rnd_brightness = tf.random_uniform((batch_size,1,1,1), -rnd_bri, rnd_bri)
...
def jpeg_compress_decompress_y(image,
                             downsample_c=True,
                             rounding=diff_round,
                             factor=1):
  image *= 255
  ...
  comp =  y_dequantize(y_comp, factor)
  comp = idct_8x8(comp)
  comp = patches_to_image(comp, height, width)

  y = comp
  image = tf.expand_dims(y, axis=-1)
