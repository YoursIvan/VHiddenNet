...
def get_img_batch(files_list,
    ...
    for i in range(batch_size):
        ...
        try:
            ...
            img_cover = np.expand_dims(img_cover,-1)
        except:
            img_cover = np.zeros((size[0],size[1],1), dtype=np.float32)
...
def main():
    ...
    parser.add_argument('exp_name', type=str,default='ivan')
    ...
    parser.add_argument('--borders', type=str, choices=['no_edge','black','random','randomrgb','image','white'], default='no_edge')
    ...
    parser.add_argument('--no_gan', action='store_true',default=False)
    ...
    parser.add_argument('--no_im_loss_steps', help="Train without image loss for first x steps", type=int, default=600)
    parser.add_argument('--no_inter_loss_steps', help="Train without inter loss for first x steps", type=int, default=600)
    ...
    image_pl = tf.placeholder(shape=[None,height,width,1],dtype=tf.float32,name="input_hide")
    ...
    loss_op, secret_loss_op, D_loss_op,G_loss,image_loss_op,lpips_loss_op , summary_op, image_summary_op, bit_accr, inter_loss, loss_inter_op = models.build_model(
            encoder=encoder,
            decoder=decoder,
    ...
    train_inter_op = tf.train.AdamOptimizer(args.lr).minimize(loss_inter_op, var_list=g_vars, global_step=global_step_tensor)
    ...
    while global_step < args.num_steps:
        ...
        for count in range(min(total_steps,args.num_steps-global_step)):
            no_im_loss = global_step < args.no_im_loss_steps
            no_inter_loss = global_step < args.no_inter_loss_steps
            ...
            if no_im_loss:
                ...
                if no_inter_loss:
                    ...
                    _, _, global_step = sess.run([train_inter_op,loss_op,global_step_tensor],feed_dict)
            ...
            if global_step % 100 ==0 :
                print('Batch Step:' + str(count) + ' loss:' + str(sess.run([loss_op],feed_dict)) + ' G_loss:' + str(sess.run([G_loss],feed_dict)) + ' bit_acc:' +  str(sess.run([bit_accr],feed_dict)) + ' image_loss:' +  str(sess.run([image_loss_op],feed_dict)) + ' lpips_loss:' +  str(sess.run([lpips_loss_op],feed_dict)) + ' inter_loss:' + str(sess.run([inter_loss],feed_dict)) )
            ...
            if global_step > 60000 and sess.run(bit_accr,feed_dict) > 0.99 and (global_step - log_index>=5000):
                ...
                tf.saved_model.simple_save(sess,
                       SAVED_MODELS + '/' + EXP_NAME + str(global_step),
